...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['book_appointment'])) {
    ...
    // Get doctor's email from the doctor's name
    $doctor_query = "SELECT email FROM doctors WHERE CONCAT(first_name, ' ', last_name) = ?";
    ...
    UNION
    SELECT doctor_name, available_date AS appointment_date, start_time, end_time
    ...
    $stmt_check = $conn->prepare($doctor_check_sql);
    $stmt_check->bind_param("ssssssss", $doctor_name, $appointment_date, $start_time, $end_time, $doctor_name, $appointment_date, $start_time, $end_time);
    $stmt_check->execute();
    $doctor_result = $stmt_check->get_result();
    ...
    WHERE email = ? 
      AND appointment_date = ? 
      AND (? < end_time AND ADDTIME(? , '01:00:00') > start_time)";
    
    // Prepare and execute the patient check query
    $stmt_patient_check = $conn->prepare($patient_check_sql);
    $stmt_patient_check->bind_param("ssss", $email, $appointment_date, $start_time, $start_time);
    $stmt_patient_check->execute();
    $patient_result = $stmt_patient_check->get_result();
    ...
    $stmt_patient->bind_param("sssssssssss", $email, $name, $phone_number, $age, $gender, $address, $appointment_date, $start_time, $patient_status, $department_name, $doctor_name);
    ...
    if ($stmt_patient->execute()) {
        ...
        $stmt_patient_id->bind_param("s", $email);
        ...
        $stmt_confirmation->bind_param("ssssssss", $doctor_email, $patient_id, $name, $appointment_date, $start_time, $end_time, $doctor_name);

        if ($stmt_confirmation->execute()) {
            header("Location: confirmation.php"); // Booking confirmed
            exit();
        } else {
            echo "Error: " . $stmt_confirmation->error;
        }
    } else {
...
$departments = [];
$department_query = "SELECT DISTINCT department FROM doctors";
$departments_result = $conn->query($department_query);
