...
// Check doctor availability for non-overlapping slots
$doctorCheckQuery = "
    SELECT * 
    FROM appointments 
    WHERE doctor_email = ? 
...
$doctorStmt->bind_param("ssss", $doctorEmail, $appointmentDate, $startTime, $endTime);
$doctorStmt->execute();
$doctorResult = $doctorStmt->get_result();

if ($doctorResult->num_rows > 0) {
    // Debug conflicting slots
    while ($row = $doctorResult->fetch_assoc()) {
        echo "Conflicting Slot: " . json_encode($row) . "<br>";
    }
    // Redirect to doctorcheck.php if a conflict exists
    header("Location: doctorcheck.php");
    exit();
} else {
    echo "Doctor is available for the selected slot.<br>";
}
...
if ($patientResult->num_rows > 0) {
    // Redirect to check.php if the patient has a conflicting appointment
    header("Location: check.php");
    exit();
}

// Insert into appointments_patients table
$insert_sql_patient = "
...
if ($stmt_patient->execute()) {
    ...
    if ($patient_id_result->num_rows > 0) {
        ...
        if ($stmt_confirmation->execute()) {
            // Redirect to confirmation.php on success
